...
Category: Tools
Tags: IC_tools, iverilog, verilator, gtkwave
...
4. verilating 时会生成两个 makefile，分别是 `V{top}.mk` 和 `V{top}_classes.mk`。其中 `V{top}_classes.mk` 会根据命令行参数给一些 verilator 预定义的变量赋值。`V{top}.mk` 作为顶层的 makefile 会自动 include 生成的 `V{top}_classes.mk` 和 `$VERILATOR_ROOT/include/verilated.mk` 文件。举个例子，如果命令行加了 `--trace` 选项，那么 `V{top}_classes.mk` 中会包含一行 `VM_TRACE = 1`，然后通过 `verilated.mk` 和 `V{top}.mk` 最终传递给 g++ 参数 `-DVM_TRACE=1`，所以我们就可以直接在 wrapper 中使用 `#if VM_TRACE` 来开启 trace 功能了
...
### Coverage
 ...
 verilator 提供了 coverage 工具，支持 line/function/toggle，需要 `sim_main.cpp` 和命令行参数配合使用，具体见 [dic_template][dic_template]和 `examples/make_trace_c`。
