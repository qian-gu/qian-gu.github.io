...
```
#!text
+5 的 机器数 = 0000-0101 ；真值 = + 000-0101

-5 的 机器数 = 1000-0101 ；真值 = - 000-0101
```
...
2. 0的表示不唯一
    ...
    + 0可以编码为两种方式： `0000-0000` 和 `1000-0000`，进一步增加了逻辑的复杂性
...
2. 缺点
    ...
    + 0的表示不唯一，0的编码仍然有两种方式： `0000-0000` 和 `1111-1111`
...
举例说明，要表示 -4 ，需要对 `0000-0100`取补码，`1111-1111 - 0000-0100 + 1 = 1-0000-0000 - 0000-0100`，相当于从2^8里面减去 4 。
...
P.S. -128 没有对应的 原码 和反码，它的补码为 `1000-0000` 。
