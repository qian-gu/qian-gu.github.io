...
Category: IC
Tags: VHDL, syntax
Slug: vhdl-notes-2-system-design
Author: Qian Gu
...
**example**
    ...
    USE work.my-package.all;   -- declaration
...
**syntax**
    ...
    #!VHDL
    label: component-name GENERIC MAP (param.list) PORT MAP (port list);
...
其中，<parameter list> 指函数的输入参数：
    ...
    <parameter list> = [CONSTANT] constant-name : constant-type;
...
其中，<parameter list> 指出了 procedure 的输入输出参数：
    ...
    #!VHDL
    <parameter list> = [CONSTANT] constant-name : mode type;
    <parameter list> = SIGNAL signal-name : mode type;
    <parameter list> = VARIABLE variable-name : mode type;
...
procedure 的调用就是它自己。
    ...
    #!VHDL
    --example
    compute-min-max (in1, in2, in3, out1, out2);
